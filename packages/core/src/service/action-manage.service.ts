...
import { V_TABLE_ACTION_MAP_TOKEN } from "./action-map.token";
...
export class ActionManager<TValue extends VTableValue = VTableValue> {
    ...
    constructor(
        @Optional()
        @Inject(V_TABLE_ACTION_MAP_TOKEN)
        _actionMap: Record<string, ActionDef<any>>,
    ) {
        const actionMap = { ...ACTION_MAP, ...(_actionMap || {}) };
        const actionNames = Object.keys(actionMap) as unknown as ActionName[];

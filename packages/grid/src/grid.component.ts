...
export class AITableGrid extends AITableGridBase implements OnInit, OnDestroy {
    ...
    private stopEvent(event: KeyboardEvent | ClipboardEvent): boolean {
        ...
        const hasAITableGrid =
            focused.querySelector('ai-table-grid') !== null || focused.closest('.ai-table-prevent-clear-selection') !== null;
        if (!hasAITableGrid) {
            return true;
        }
    ...
    private copyCells(dataTransfer?: DataTransfer | null) {
    ...
    private pasteCells(dataTransfer?: DataTransfer | null) {
        writeToAITable(this.aiTable, this.actions, dataTransfer).then((result) => {
            ...
            if (isPasteOverMaxRecords || isPasteOverMaxFields) {
                const message = getI18nTextByKey(
                    this.aiTable,
                    isPasteOverMaxRecords ? AITableGridI18nKey.pasteOverMaxRecords : AITableGridI18nKey.pasteOverMaxFields
                );

...
export class AITableCellProgress extends CoverCellBase {
    ...
    readonly whiteBgConfig = computed(() => {
        ...
        return {
            x: x - groupIconOffset - AI_TABLE_CELL_PADDING + AI_TABLE_CELL_BORDER / 2,
    ...
    pointerDragmove(e: KoEventObject<MouseEvent>) {
        ...
        const { x, groupIconOffset = 0 } = render;
        ...
        const dragX = point.x - columnLeftX - x - groupIconOffset;
        const minX = x;
        const maxX = x + this.railWidth();

        let dragPointerX = point.x - columnLeftX - groupIconOffset;

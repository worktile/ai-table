...
import { AITableRender, AITableSelectField, AITableWrapTextData } from '../../types';
...
export class CellHelper extends Drawer {
    ...
    private renderCellSelect(render: AITableRender, ctx?: any) {
        ...
        if ((field as AITableSelectField).settings.is_multiple) {
            ...
            this.renderCellSingleSelect(render, ctx);
        }
    }

    private renderCellSingleSelect(render: AITableRender, ctx?: any) {
        ...
        const maxTextWidth = isOperating
            ? columnWidth - 2 * (AI_TABLE_CELL_VALUE_PADDING + AI_TABLE_OPTION_ITEM_PADDING) - AI_TABLE_CELL_DELETE_ITEM_BUTTON_SIZE - 12
            : columnWidth - 2 * (AI_TABLE_CELL_VALUE_PADDING + AI_TABLE_OPTION_ITEM_PADDING);
        ...
        if (ctx) {
            ...
            this.label({
                x: x + AI_TABLE_CELL_VALUE_PADDING,

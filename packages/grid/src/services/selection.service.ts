...
import { AIRecordFieldIdPath, AITable, AITableDrag, DragType } from '../core';
...
export class AITableGridSelectionService {
    ...
    drag(config: AITableDrag) {
        this.aiTable.drag!.set(config);
    }

    clearDrag() {
        this.aiTable.drag!.set({
            type: DragType.none,
            sourceIds: new Set()
        });
    }
